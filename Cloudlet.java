...
public class Cloudlet {
	 ...
	 * @post $none
	 */
	public double[][] expectedCompletionNew(List<Cloudlet> clist,List<Vm> vmlist ,int vmn,int cn){
		double kk[][]=new double[100][100];
		for(int i=0;i<vmn;i++){
			for(int j=0;j<cn;j++){
				kk[i][j]=((clist.get(j).cloudletLength/vmlist.get(i).getMips())+(clist.get(j).getUtilizationOfRam(j)/vmlist.get(i).getCurrentRequestedMaxMips()))*100;
			}
		}
